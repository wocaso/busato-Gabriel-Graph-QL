...
describe("Test de endpoint /Productos con Asserts", async function() {
    ...
    it("Post a product", async function() {
        const newProduct = { title: "Test Mocha Product", price: 30000, thumb: "http://www.axios.com/thumb.jpg"};
        const response = await req.post(`/productos`).send(newProduct);
        assert.deepStrictEqual(response.body, {id: response.body.id, ...newProduct})
    ...
    it("Update Product", async function() {
        ...
        response = await req.put(`/productos/${insertedProduct.id}`).send(insertedProduct);
