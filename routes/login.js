...
loginRouter.post("/login", passport.authenticate("login", { failureRedirect: "/loginError" }), postLogin);
loginRouter.get("/loginError", getLoginError);

loginRouter.get("/register", getRegister);
loginRouter.post("/register", passport.authenticate("register", { failureRedirect: "/registerError", successRedirect: "/login" }))
